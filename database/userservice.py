...
def register_user_db(name, phone_number, level):
    ...
    if checker:
        return f"There is already this phone number and here his ID {checker.id}"
    else:
        new_user = User(name=name, phone_number=phone_number, level=level,
                        datetime=datetime.now())
        db.add(new_user)
        db.commit()
        return f"Successfully done, here is ID of new user {new_user.id}"
